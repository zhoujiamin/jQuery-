...
//看到这里的时候很疑惑，为什么明明两个函数相等，但使用效果缺不同，this在这里非常重要
//对于jQuery.extend，函数内的this指的是jQuery
//对于jQuery.fn.extend,this指的是调用函数的实例
	...
	for ( ; i < length; i++ ) {
		...
		if ( ( options = arguments[ i ] ) != null ) {
			...
			for ( name in options ) {
				...
				// Prevent never-ending loop 防止有环，例如 extend(true, target, {'target':target});
			  //一旦target对象中，增加了target属性后，{'target':target}中的target中也会增加target属性的嵌套
				if ( target === copy ) {
					continue;
				}

				// Recurse if we're merging plain objects or arrays 递归调用
				if ( deep && copy && ( jQuery.isPlainObject( copy ) ||
					...
					// Never move original objects, clone them 进行递归复制
...
var rootjQuery,
	...
	init = jQuery.fn.init = function( selector, context, root ) {//context用于限制搜索范围，
		var match, elem;
     //如果是以下几种情况就返回this 对于new jQuery.fn.init(null),返回的就是这个jQuery.fn.init对象
...
init.prototype = jQuery.fn;//这句话非常重要，使得通过jQuery.fn.init生成的实例可以访问到jQuery原型函数
